...
export function PraisesRankingPage() {
  ...
  async function fetchAllMusics() {
    const { data }: { data: MusicData[] } = await api.get('/api/praises')
  ...
  async function handleVote(musicId: string, newMusic: MusicData | undefined = undefined) {
    const userVoteData = localStorage.getItem('ranking-louvores-hebert-v1@userVoteData')
    let voteData: Vote[] = []
    ...
    localStorage.setItem('ranking-louvores-hebert-v1@userVoteData', JSON.stringify(voteData))
    let updatedMusics: MusicData[]

    if (newMusic) {
      newMusic.votes = 1
      updatedMusics = [...musics, newMusic]
      await api.post('/api/praises', newMusic)
    } else {
      updatedMusics = await Promise.all(
        musics.map(async oldMusic => {
          ...
          if (musicId === oldMusic.id) {
            ...
            await api.put(`/api/praises/${updatedMusic.id}`, updatedMusic)
          }
  ...
  return (
    <div>
      ...
      <main>
        <h1>Aqui os fãs de Hebert Freire têm o poder de escolher os próximos louvores do curso!</h1>
